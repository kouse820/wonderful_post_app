class SampleArticlesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @sample_article.update(sample_article_params)
        format.html { redirect_to @sample_article, notice: "Sample article was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to sample_articles_path, notice: "Sample article was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_sample_article
      @sample_article = SampleArticle.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def sample_article_params
      params.expect(sample_article: [ :title, :content ])
    end
end
